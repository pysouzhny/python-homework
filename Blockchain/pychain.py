...
#  which is set equal to a `Record` that contains the `sender`, `receiver`, and `amount` values. The updated `Block`should also include the attributes for `creator_id` and `prev_hash`.
...
# YOUR CODE HERE
input_receiver=st.text_input('receiver name')
# @TODO:
# Add an input area where you can get a value for `amount` from the user.
# YOUR CODE HERE
input_amount=st.number_input('amount to be transacted')
